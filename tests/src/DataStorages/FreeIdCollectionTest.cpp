...
TEST_F(FreeIdCollectionTest, testClear) {
	...
	int v1_id = coll->addData(v1);
	int v2_id = coll->addData(v2);

	coll->clear();
	ASSERT_EQ(coll->getCount(), 0);
...
TEST_F(FreeIdCollectionTest, testDelete) {
	...
	ASSERT_EQ(coll->getCount(), 1);
	ASSERT_EQ(coll->contains(v1_id), true);
	ASSERT_EQ(coll->removeData(v1_id), true);

	ASSERT_EQ(coll->removeData(v1_id), false);
	int v2 = INT_MIN;
	coll->addData(v2);
	ASSERT_EQ(coll->removeData(v1_id), true);
...
TEST_F(FreeIdCollectionTest, testDeleteMinKey) {
	...
	int v1_id = coll->addData(v1);
	ASSERT_EQ(coll->getCount(), 1);
	(void)coll->addData(v2);
	ASSERT_EQ(coll->getCount(), 2);
	ASSERT_EQ(coll->removeData(v1_id), true);
	ASSERT_EQ(coll->getCount(), 1);
	int v3_id = coll->addData(v3);
